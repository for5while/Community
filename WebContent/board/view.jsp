...
// 비밀글인데 글쓴이가 아니고 관리자도 아닐 때
if(bb.getOption() == 1 && !bb.getMemberId().equals(memberID) && !memberID.equals("admin")) {

	// 글 번호 기준 세션 없거나 값이 다를 때
	if(session.getAttribute("secretTable") != table && (Integer)session.getAttribute("secretID") != id) {
		out.print("<script type='text/javascript'>");
		out.print("alert('비정상적인 접근입니다.dd');");
		out.print("location.href = '../index.jsp';");
		out.print("</script>");
		return;
	}
	...
	<div class="view">
		...
		<div class="foot">
			<div class="btn">
				<%
				// 글 작성자 본인이라면
				if(memberID != null && (memberID.equals(bb.getMemberId()) || memberID.equals("admin"))) {
				%>
				<div class="left">
			...
			<div class="comment">
				...
				ArrayList<BoardBean> commentList = bdao.getCommentList(startRow, pageSize, id, table);
				BoardBean bestRecommend = bdao.bestRecommend(table, id); // 베스트 댓글
				%>
						...
						<div class="co_content">
							...
							<div class="text_top">
								<span class="text_id">
									...
									<% if(commentList.get(i).getMemberId().equals(bb.getMemberId())) { %><span class="co_author" title="글쓴이">글쓴이</span><% } %>
								</span>
